...
const updateCandidatura = async (id, updates) => {
    ...
    try {
      ...
      // 2. Local State Update (Assicurati che 'c' sia usato correttamente)
      setCandidature(prevList => prevList.map(c => 
        c.id === id ? { ...c, ...updates } : c
      ));

      // 3. Logica Gamification e Notifiche
      if (updates.stato && updates.stato !== prev?.stato) {
        if (updates.stato === 'Colloquio') {
          ...
          const pBody = _l === 'en' ? `Ready for ${prev?.azienda}? Checklist active! 💜` : `Pronto per ${prev?.azienda}? Checklist attivata! 💜`;
          ...
          showToast(_l === 'en' ? '🏆 OFFER! +50 XP' : '🏆 OFFERTA RICEVUTA! +50 XP', 'success');
          triggerConfetti();
        } 
...
const computeStatsFrom = (list) => {
    // Se la lista è vuota, resettiamo tutto
  ...
  !['GHOSTED', 'Rifiutata'].includes(c.stato)
    ...
    // --- FIX CRASH NOTIFICHE (ERRORE #300) ---
    ...
    return { 
      ...
      weekStreak, referral: referralCount 
    }
  }
