  ...
  if (showNotifs) return (
    <div className="screen">
      <div className="flex items-center gap-3 px-5 pt-safe pt-4 pb-3 border-b border-white/5 flex-shrink-0">
        ...
        <h2 className="font-bold text-txt">{isIt ? 'Notifiche' : 'Notifications'}</h2>
      </div>
      <div className="flex-1 scrollable px-4 py-4">
        {notifications.length === 0 ? <p className="text-center text-muted mt-10">Zzz...</p> : 
     ...
     <div className="flex-1 scrollable px-4 pb-8 space-y-4">
        ...
        <div className="card">
          ...
          <div className="flex items-center mt-3">
            {streak > 1 && <p className="text-sm font-bold text-amber">🔥 {streak} {isIt ? 'giorni di fila' : 'day streak'}</p>}
  ...
  <p className="text-[11px] text-muted mb-3 leading-relaxed">
    {t('profile.import_description')}
        ...
        <div className="pt-4 space-y-2">
          <button onClick={() => signOut()} className="w-full py-3 bg-white/5 rounded-xl text-txt font-bold text-sm">LOGOUT</button>
          <button onClick={deleteAccount} className="w-full py-2 text-red/60 text-[10px] font-medium uppercase tracking-widest text-center">{isIt ? '🗑️ ELIMINA ACCOUNT' : '🗑️ DELETE ACCOUNT'}</button>
        </div>
      </div>
    </div>
  )
