...
export default function Profile() {
  ...
  const downloadTemplate = () => {
    ...
    a.href = url; a.download = 'template_candidature.xlsx'
  ...
  const deleteAccount = async () => {
    if (confirm(t('profile.confermaElimina'))) {
  ...
  if (showNotifs) return (
    <div className="screen">
      ...
      <div className="flex-1 scrollable px-4 py-4">
        {notifications.length === 0 ? <p className="text-center text-muted mt-10">Nessuna notifica</p> : 
      ...
      <div className="flex-1 scrollable px-4 pb-8 space-y-4">
        ...
        <div className="card">
          ...
          <div className="grid grid-cols-4 gap-2 mt-3">
            {BADGES.map(badge => {
              ...
              return (
                <div key={badge.id} className={`p-2 rounded-xl text-center flex flex-col items-center ${isEarned ? 'bg-purple/20' : 'opacity-20'}`}>
                  {isEarned ? (
                    <div className="w-8 h-8 mb-1" dangerouslySetInnerHTML={{ __html: badge.svg }} />
                  ) : (
                    <div className="w-8 h-8 mb-1 flex items-center justify-center text-xl font-bold text-muted">?</div>
                  )}
        ...
        <div className="card">
          ...
          <div className="space-y-2">
            <button onClick={downloadTemplate} className="w-full py-2 bg-white/5 rounded-xl text-xs font-semibold border border-white/10">1. {t('profile.scaricaTemplate')}</button>
            <button onClick={() => fileRef.current?.click()} className="w-full py-2 bg-purple rounded-xl text-xs font-bold text-white">2. {t('profile.caricaExcel')}</button>
        ...
        <div className="pt-4 space-y-2">
          <button onClick={() => signOut()} className="w-full py-3 bg-white/5 rounded-xl text-txt font-bold text-sm">{t('profile.logout')}</button>
          <button onClick={deleteAccount} className="w-full py-2 text-red/60 text-[10px] font-medium uppercase tracking-widest">{t('profile.eliminaAccount')}</button>
        </div>
      </div>
    </div>
  )
